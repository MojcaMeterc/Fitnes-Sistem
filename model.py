...
class Uporabnik:
    ...
    def prijava(conn, email, geslo):
        ...
        if vrstica := cur.fetchone():
            return Uporabnik(
                conn, vrstica[1], vrstica[2], vrstica[3], vrstica[4], vrstica[0]
            )
        return None
...
class Trener:
    ...
    def pridobi_po_id(conn, trener_id):
        ...
        if vrstica := conn.execute(sql, (trener_id,)).fetchone():
            return Trener(
                conn,
                vrstica["ime"],
                vrstica["priimek"],
...
class Admin:
    ...
    def vsi_trenerji(self):
        return self.conn.execute("SELECT * FROM trener ORDER BY priimek").fetchall()
    ...
    def izbrisi_trenerja(self, trener_id):
        self.conn.execute("DELETE FROM trener WHERE trener_id = ?", (trener_id,))
        self.conn.commit()
